...
- (void)logFormat:(NSString *)format, ... {
    ...
    @try {
        ...
        NSLog(@"[CameraDiagnostic] Erro ao formatar log: %@", exception);
    }
}
...
- (void)logError:(NSString *)format, ... {
    ...
    @try {
        ...
        NSLog(@"[CameraDiagnostic] Erro ao registrar erro: %@", exception);
    }
}
...
- (void)logWarning:(NSString *)format, ... {
    ...
    @try {
        ...
        NSLog(@"[CameraDiagnostic] Erro ao registrar aviso: %@", exception);
    }
}
...
- (void)logInfo:(NSString *)format, ... {
    ...
    @try {
        ...
        NSLog(@"[CameraDiagnostic] Erro ao registrar info: %@", exception);
    }
}
...
- (void)logDebug:(NSString *)format, ... {
    ...
    @try {
        ...
        NSLog(@"[CameraDiagnostic] Erro ao registrar debug: %@", exception);
    }
}
...
- (void)logVerbose:(NSString *)format, ... {
    ...
    @try {
        ...
        NSLog(@"[CameraDiagnostic] Erro ao registrar verbose: %@", exception);
    }
}
...
- (void)logJSONData:(NSDictionary *)jsonData forCategory:(LogCategory)category withDescription:(NSString *)description {
    ...
    @try {
        ...
        NSLog(@"[CameraDiagnostic] Erro ao registrar JSON: %@", exception);
    }
}
...
- (void)startNewSession {
    @try {
        ...
        NSLog(@"[CameraDiagnostic] Erro ao iniciar nova sessão: %@", exception);
        [_sessionLock unlock]; // Garantir que o lock é liberado em caso de exceção
    }
}

- (void)startNewSessionForApp:(NSString *)appName bundleId:(NSString *)bundleId {
    if (!appName || !bundleId) return;
    ...
    @try {
        ...
        // Criar nome de arquivo específico para o app
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
        [dateFormatter setDateFormat:@"yyyyMMdd_HHmmss"];
        NSString *timestamp = [dateFormatter stringFromDate:[NSDate date]];
        
        // Sanitizar nomes de arquivo
        NSString *safeAppName = [appName stringByReplacingOccurrencesOfString:@"/" withString:@"_"];
        NSString *safeBundleId = [bundleId stringByReplacingOccurrencesOfString:@"/" withString:@"_"];
        
        _currentSessionFile = [NSString stringWithFormat:@"%@/%@_%@_%@%@",
                              _logDirectory, safeAppName, safeBundleId, timestamp, kLogFileExtension];
        
        // Iniciar nova sessão com informações do app
        _sessionData = [NSMutableDictionary dictionary];
        _sessionData[@"sessionInfo"] = @{
            @"id": [[NSUUID UUID] UUIDString],
            @"startTime": [NSDate date].description,
        ...
        [self logInfo:@"Nova sessão iniciada para %@ (%@)", appName, bundleId];
    } @catch (NSException *exception) {
        NSLog(@"[CameraDiagnostic] Erro ao iniciar sessão para app: %@", exception);
        [_sessionLock unlock]; // Garantir que o lock é liberado em caso de exceção
    }
}
...
- (void)setSessionValue:(id)value forKey:(NSString *)key {
    ...
    @try {
        ...
        NSLog(@"[CameraDiagnostic] Erro ao definir valor de sessão: %@", exception);
    }
}
...
- (NSDictionary *)currentSessionData {
    @try {
        ...
        NSLog(@"[CameraDiagnostic] Erro ao obter dados atuais da sessão: %@", exception);
        [_sessionLock unlock];
        return @{};
    }
}
...
- (void)finalizeSessionWithCompletion:(void(^)(NSURL *fileURL, NSError *error))completion {
    @try {
        ...
        if (completion) {
            NSError *error = [NSError errorWithDomain:@"com.cameradiagnostic.logger"
                                                 code:1
                                             userInfo:@{NSLocalizedDescriptionKey: @"Erro ao finalizar sessão"}];
...
- (void)appendToSessionData:(NSDictionary *)data {
    ...
    @try {
        ...
        NSLog(@"[CameraDiagnostic] Erro ao anexar dados à sessão: %@", exception);
        [_sessionLock unlock];
    }
}
...
- (void)saveCurrentSessionData {
    ...
    @try {
        ...
        NSLog(@"[CameraDiagnostic] Erro ao salvar dados da sessão: %@", exception);
        [_sessionLock unlock];
    }
}
...
void logMessage(NSString *message, LogLevel level, LogCategory category) {
    ...
    @try {
        ...
        NSLog(@"[CameraDiagnostic] Erro ao registrar mensagem: %@", exception);
    }
}
...
void logJSONWithDescription(NSDictionary *jsonData, LogCategory category, NSString *description) {
    ...
    @try {
        ...
        NSLog(@"[CameraDiagnostic] Erro ao registrar JSON com descrição: %@", exception);
    }
}
...
void startNewLogSession(void) {
    @try {
        ...
        NSLog(@"[CameraDiagnostic] Erro ao iniciar nova sessão de log: %@", exception);
    }
}
...
void finalizeLogSession(void) {
    @try {
        ...
        NSLog(@"[CameraDiagnostic] Erro ao finalizar sessão de log: %@", exception);
    }
}
...
void setLogSessionValue(NSString *key, id value) {
    ...
    @try {
        ...
        NSLog(@"[CameraDiagnostic] Erro ao definir valor para sessão: %@", exception);
    }
}
...
void startNewLogSessionForApp(NSString *appName, NSString *bundleId) {
    ...
    @try {
        [[DiagnosticLogger sharedInstance] startNewSessionForApp:appName bundleId:bundleId];
    } @catch (NSException *exception) {
        NSLog(@"[CameraDiagnostic] Erro ao iniciar sessão para app: %@", exception);
    }
}
